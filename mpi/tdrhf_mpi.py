...
def main(argv):
	...
	parser = argparse.ArgumentParser(description='Time Dependent Hartree Fock ')
	...
	# export
	parser.add_argument('-nb_print_mo',default = 2, help='Number of orbitals to be written',type=int)
	...
	# only the master proc computes the dynamics
	if rank == 0:

		print '\n\t Compute the ground state HF Ground State\n\t',
		print '-'*50
		L,C,Cp,F0,F0p,D,Dp,P,X = rhf(mol,bfs,S,Hcore,Ints,MaxIter=args.MaxIter,eps_SCF=args.eps_SCF)
		...
		if _test_:
			...
			if _verbose_:
				...
				print'\t\t AO Fock Matrix'
				print F0

				print'\t\t Low Fock Matrix'
				print F0p

				print'\t\t AO Density Matrix'
				print D

				print'\t\t Low Density Matrix'
				print Dp
		...
		if _test_:
			#print'\n'
			#print'='*40
			print'\t\t Verify the basis transformation from ORTHO basis to AO basis ',
			#print'='*40
		...
		##########################################################
		if _plot_:
			plot(Tplot,FIELD,N,Q,MU,cutlow=0.005,cuthigh=0.995)
